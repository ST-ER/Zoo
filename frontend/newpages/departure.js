...
const headers = ["ID_Изделия", "Категория", "Цена за ед.", "Длительность(мин)", "Наименование"];

function fillClientData(obj, arr) {
    obj.id_item = arr[0];
    obj.id_category = arr[1];
    obj.priceperitem = arr[2];
    obj.processingtime = arr[3];
    obj.itemname = arr[4];
}
...
async function getItems(){
    ...
    if(response.ok){
        ...
        const itemDiv = document.getElementById("table");
        ...
        for(let i = 0; i < json.length; i++){
            ...
            tr.setAttribute("id", "trID_" + json[i].id_item);
            ...
            deleteBtn.dataset.id_item = json[i].id_item;
            ...
            updateBtn.dataset.id_item = json[i].id_item;
            console.log(updateBtn.dataset.id_item)
        ...
        deleteBtns.forEach((btn) =>{
            btn.addEventListener("click", () =>{
                const id_item = btn.getAttribute("data-id_item");
                console.log(id_item)
                const deletion = confirm("Удалить запись?")
                if(deletion){
                    deleteField(route, id_item);
                    const trID = document.getElementById("trID_" + id_item);
                    trID.remove();
        ...
        updateBtns.forEach((btn) =>{
            btn.addEventListener("click", () =>{
                ...
                const id_item = btn.getAttribute("data-id_item");
                console.log(id_item)
                const trInputs = document.querySelectorAll(`#trID_${id_item} input`);
